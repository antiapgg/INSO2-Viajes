...
@Named
@ViewScoped
public class BuscarZonasController implements Serializable {
    ...
    void init(){
        ...
        System.out.println("ENTRO EN BUSCAR ZONAS CONTROLLER");
        
        FacesContext fc = FacesContext.getCurrentInstance();
        Map<String, String> params = fc.getExternalContext().getRequestParameterMap();
        
        if(params.size() > 0){
            Integer zonaId = new Integer(params.get("zonaId"));
            System.out.println(zonaId);
            zona = zonasEJB.find(zonaId);
            
            listaDestinos = showDestinos(zona);
        }
    ...
    public List<Destino> showDestinos(Zona zona){
        listaDestinos = destinosEJB.consultarZonas(zona);
